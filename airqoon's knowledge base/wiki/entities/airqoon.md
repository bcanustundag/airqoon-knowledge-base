---
title: "Airqoon"
type: entity
entity_type: company
tags: [airqoon, company, air-quality, iot, turkey]
created: 2026-04-23
updated: 2026-04-23
sources: [product-page.md, unite-l.md, airqoon-vs-oizom-comparison.md, airqoon-lens.md, distributor-partnership-search.md, customer-success-scenarios.md, Use-Cases.md, satis-arastirma.md]
---

# Airqoon

> Turkish environmental technology company specialising in low-cost, real-time air quality monitoring and management.

---

## Identity

| Field | Value |
|-------|-------|
| **Type** | Product & Solution Company |
| **Domain** | Air quality monitoring, environmental data intelligence |
| **HQ** | Turkey |
| **Founders** | [[Baris Can Ustundag]] (Co-founder), others unnamed in sources |
| **Active markets** | Turkey (20+ cities), Azerbaijan, Iraq, Egypt (prospecting), Australia (prospecting) |
| **Website** | airqoon.com |

---

## Product Portfolio

### Hardware
- **[[Unit L]]** — Flagship outdoor air quality sensor (3.2 kg, solar-powered, 14-day battery)
- **[[Unit M]]** — Compact/indoor unit with noise monitoring capabilities
- **Unit N** — Planned unit (details TBD)

### Software
- **[[Airqoon Lens]]** — Customer analytics platform (real-time dashboards, compare mode, reports, alerts, API)
- **[[Airqoon Lens AI]]** — GenAI-backed monthly assessment reports, pollution source detection, trend analysis
- **[[Airqoon Map]]** — Public-facing real-time air quality map with citizen engagement features

---

## Value Proposition

1. **Cost leadership** — $2,500–5,500 vs >$8,000 for competitors; 40–60% savings
2. **Operational simplicity** — 5-minute deployment, modular sensor cartridges, 14-day battery backup
3. **Full-stack intelligence** — hardware + cloud analytics + AI reporting in one ecosystem
4. **Community engagement** — public map with citizen complaint collection (unique differentiator vs competitors)
5. **Regulatory readiness** — aligns with CEN TS 17660, EU Directive 2008/50/EC indicative monitoring standards

---

## Customer Base (Known)

| Customer | Type | Context |
|----------|------|---------|
| Akçansa | Cement industry | BCM/CNK regions, perimeter monitoring |
| EnerjiSA Üretim | Energy | Calibration reports, daily alerts, EBRD/IFC compliance |
| Kadıköy Belediyesi | Municipality | Map export, polygon display, device management |
| Bursa Büyükşehir Belediyesi | Municipality | Air quality monitoring network |
| GİSAŞ | Industrial zone | Short-time monitoring, regulatory compliance |

---

## Distribution & Partnerships

Active distributor search across: Egypt (HAK Automation, ELS, Gemica Engineering), Saudi Arabia (REDA Safe), Australia (Alpha Scientific, Connected IoT), Europe (Codico, Calectro, Atlantik Elektronik, Casella Solutions), Finland (Stick Wahlstrom).

Consulting/enterprise partnerships explored with: Bureau Veritas, DNV, TÜV SÜD, Deloitte, PwC, KPMG, EY, McKinsey, Arcadis.

---

## Key Projects

- **[[PACE Projesi]]** — EU/GIZ-funded earthquake recovery project; air monitoring in 11 affected Turkish provinces
- **GEFF Turkey** — Feasibility study (green finance)
- **SCEWC 2023** — Smart City Expo World Congress participation / lead generation

---

*See also: [[wiki/overview]], [[Unit L]], [[Airqoon Lens]], [[Oizom]]*
